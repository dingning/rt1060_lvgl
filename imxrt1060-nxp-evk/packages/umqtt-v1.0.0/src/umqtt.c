...
void mqtt_thread1(void *params)
{
    //rt_kprintf("mqtt_thread1\n");
    struct umqtt_client *client = (struct umqtt_client *)params;
    while (1)
    {
        umqtt_keepalive_callback(client);
        umqtt_reconnect_callback(client);
        pubrec_cycle_callback(client);
        rt_thread_mdelay(UMQTT_INFO_DEF_UPLINK_TIMER_TICK);
    }
...
#if 1
    rt_thread_t th1 = rt_thread_create(_name, mqtt_thread1,  (void *)mqtt_client, mqtt_client->mqtt_info.thread_stack_size, 
