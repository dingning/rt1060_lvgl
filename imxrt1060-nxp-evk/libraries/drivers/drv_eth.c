...
static status_t _ENET_SendFrame(ENET_Type *base, enet_handle_t *handle, const uint8_t *data, uint32_t length)
{
    ...
    curBuffDescrip = handle->txBdRing[0].txBdBase;
    ...
    //curBuffDescrip = handle->txBdCurrent[0];
    //curBuffDescrip =handle->txBdRing[0].txBdBase[0];

        /* Increase the buffer descriptor address. */
        if (curBuffDescrip->control & ENET_BUFFDESCRIPTOR_TX_WRAP_MASK)
        {
            //handle->txBdCurrent[0] = handle->txBdBase[0];
            handle->txBdRing[0].txBdBase[0] = handle->txBdRing[0].txBdBase[0];
        }
        else
        {
            //handle->txBdCurrent[0]++;
            handle->txBdRing[0].txBdBase++;
        }
    ...
    if (length != 0)
    {
        ...
        if (p != NULL)
        {
           // status = ENET_ReadFrame(enet_base, enet_handle, p->payload, length);
            uint32_t ts;
       status = ENET_ReadFrame(enet_base, enet_handle, NULL, 0, 0,&ts);
...
ENET_GetRxErrBeforeReadFrame(enet_handle, error_statistic,0);
      ...
      uint32_t ts;
       ENET_ReadFrame(enet_base, enet_handle, NULL, 0, 0,&ts);
    }
...
#else
    rt_phy_status status = phy_dev->ops->init(imxrt_eth_device.enet_base, 0, CLOCK_GetFreq(kCLOCK_AhbClk));
#endif
